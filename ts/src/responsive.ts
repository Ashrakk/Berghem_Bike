export class ResponsiveManager {
  ...
  private allColumnsElementsOriginalClasses: DOMTokenList[] = [];
  ...
  constructor() {
    ...
    //fix page
    window.addEventListener('resize', (evt: UIEvent) => {this.handleResponsiveness(evt);})
  ...
  private mobileMode(status: boolean) //toggle between mobile and desktop mode
  {
    let index = 0;
    if(status === true && status !== this.mobileStatus)
    {
      this.allColumnsElements?.forEach((elem) => 
      {
      ...
      this.allRowElements?.forEach((elem) => 
      {
    ...
    else if (status === false && status !== this.mobileStatus)
    {
      this.allColumnsElements?.forEach((elem) => 
      {
        ...
        this.allColumnsElementsOriginalClasses[index].forEach(
  ...
  private handleResponsiveness(event?: UIEvent)
  {
  ...
  private handleMenuButton()
  {
    ...
    {
      this.mainContentContainer?.classList.remove('div_center_menu_open');
      this.menuNavbar?.classList.remove('div_navbar_menu_open', 'anim_menuFadeIn');
      ...
      {
        this.menuOpenerContainer?.insertAdjacentElement('beforeend', this.menuOpener);
